...
def main(envName,baseline,saveFileName,mode,gamma,GAELambda,nHistory,minSd,nEpochsPerIter,verbose,plotProgress,useScaler,globalVar,iterSimBudget,maxSimSteps,nBenchmarkRuns,scale,rewardScale,L2Critic,networkWidth,actionRepeat,linearSdAnneal,nEpisodes):
    test=envName
    ...
    if saveFileName is None:
        saveFileName="results/{}_{}_randomized.log".format(envName,mode)
    ...
    def logResults(envName,learningRate,networkWidth,networkActivation,nEpisodes,iterSimBudget,nEpochs,minibatchSize,actionRepeat,meanEpisodeReward,bestSoFar):
        writeHeader=not isfile(saveFileName); 
        with open(saveFileName, 'a') as file:
            if writeHeader:
                file.write('Env, learningRate, networkWidth, networkActivation, nEpisodes, iterSimBudget, nEpochs, minibatchSize, actionRepeat, meanEpisodeReward, bestSoFar \n')
            file.write('{},{},{},{},{},{},{},{},{},{},{} \n'.format(envName,learningRate,networkWidth,networkActivation,nEpisodes,iterSimBudget,nEpochs,minibatchSize,actionRepeat,meanEpisodeReward,bestSoFar))


    #Everything runs multiple times to get reliable performance estimate
    for benchmarkRunIdx in range(nBenchmarkRuns):
        #deterministic random seeds to allow running the same script with different environments and always have the same sequence of hyperparameter combinations
        random.seed(benchmarkRunIdx*997)  
        bestAvgEpRew=-np.inf

        #randomize parameters
        #TODO: randomize action repeat as well
        actionRepeat=random.randint(1,2)
        learningRate=np.power(10,-(3.0+random.random()))                             #learning rate
        networkWidth=random.randint(16,256)
        networkActivation="tanh" if random.randint(0,1)==0 else "lrelu"
        iterSimBudget=random.randint(2000,64000)
        nEpochs=random.randint(1,20)
        minibatchSize=random.randint(32,1024)               #training minibatch size
        ...
        critic=Critic(stateDim=stateDim+1,learningRate=learningRate,nHidden=networkDepth,networkUnits=networkWidth,networkActivation=networkActivation,useSkips=networkSkips,lossType="L2" if L2Critic else "L1")
        ...
        policy.init(sess,0,1,0.5*(actionMinLimit+actionMaxLimit)*np.ones(actionDim),1.0*(actionMaxLimit-actionMinLimit)*np.ones(actionDim),minibatchSize,4000)
        ...
        #at the end of a training run, log results
        bestAvgEpRew=max([bestAvgEpRew,benchmarkResult])
        logResults(envName=envName,learningRate=learningRate,networkWidth=networkWidth,networkActivation=networkActivation,nEpisodes=nEpisodes,iterSimBudget=iterSimBudget,nEpochs=nEpochs,minibatchSize=minibatchSize,actionRepeat=actionRepeat,meanEpisodeReward=benchmarkResult,bestSoFar=bestAvgEpRew)
    print("All done!")
...
if __name__ == "__main__":
    ...
    parser.add_argument('-l', '--GAELambda', type=float, help='Lambda for Generalized Advantage Estimation',
                        default=0.95)
    parser.add_argument('-H', '--nHistory',type=int, help='number of iterations to use for training variance',default=5)
    ...
    parser.add_argument('--iterSimBudget', type=int, help='iteration simulation budget',default=8*2048)
